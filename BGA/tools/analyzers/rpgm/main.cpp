...
int main(int argc, char *argv[])
{
    ...
    QCoreApplication::setApplicationName(QStringLiteral("BGA RPGM Analyzer"));

    QCommandLineParser parser;
    parser.setApplicationDescription(QStringLiteral("Extracts text assets from RPG Maker projects."));
    parser.addHelpOption();
    parser.addVersionOption();

    QCommandLineOption projectOption(QStringList{QStringLiteral("p"), QStringLiteral("project")},
                                     QStringLiteral("Path to RPG Maker project directory."),
                                     QStringLiteral("path"));
